...
test(async function timeAxisBottom() {
...
test(async function timeAxisTop() {
  return svg`<svg width=640 height=${domains.length * 60}>${domains.map(
    (domain, i) =>
      svg`<g transform="translate(0,${i * 60})">${Plot.plot({
        marginTop: 40,
...
test(async function timeAxisLeft() {
...
test(async function timeAxisRight() {
  const somedomains = domains.filter((d, i) => i % 3 === 0);
  return svg`<svg height=400 width=${somedomains.length * 80}>${somedomains.map(
    (domain, i) =>
      svg`<g transform="translate(${i * 80},0)">${Plot.plot({
...
test(async function timeAxisExplicitInterval() {
...
test(async function timeAxisExplicitNonstandardInterval() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
...
test(async function timeAxisExplicitNonstandardIntervalTicks() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
...
test(async function timeAxisOrdinal() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
    x: {interval: "month"},
...
test(async function warnTimeAxisOrdinalIncompatible() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
...
test(async function timeAxisOrdinalSparseTicks() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
...
test(async function timeAxisOrdinalSparseInterval() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
    x: {interval: "52 weeks"},
    marks: [Plot.barY(aapl, Plot.groupX({y: "median", title: "min"}, {title: "Date", x: "Date", y: "Close"}))]
...
test(async function timeAxisOrdinalTicks() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return Plot.plot({
...
test(async function warnTimeAxisOrdinalExplicitIncompatibleTicks() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
...
test(async function timeAxisLocal() {
  const dates = [
