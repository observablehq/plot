...
async function pointermove(plot, p = {clientX: 140, clientY: 70}) {
  plot.dispatchEvent(Object.assign(new Event("pointermove"), p));
...
export async function tipAreaStack() {
  ...
  return pointermove(
    Plot.areaY(industries, {x: "date", y: "unemployed", fill: "industry", tip: true}).plot({marginLeft: 50})
  );
}
...
export async function tipBin() {
  const olympians = await d3.csv<any>("data/athletes.csv", d3.autoType);
  return pointermove(Plot.rectY(olympians, Plot.binX({y: "count"}, {x: "weight", tip: true})).plot());
}

export async function tipBinStack() {
  const olympians = await d3.csv<any>("data/athletes.csv", d3.autoType);
  return pointermove(Plot.rectY(olympians, Plot.binX({y: "count"}, {x: "weight", fill: "sex", tip: true})).plot());
}
...
export async function tipDodge() {
  ...
  return pointermove(
    Plot.dot(penguins, Plot.dodgeY({x: "culmen_length_mm", r: "body_mass_g", tip: true})).plot({height: 160})
  );
}
...
export async function tipDot() {
  const penguins = await d3.csv<any>("data/penguins.csv", d3.autoType);
  return pointermove(
    Plot.dot(penguins, {x: "culmen_length_mm", y: "culmen_depth_mm", stroke: "sex", tip: true}).plot()
  );
}
...
export async function tipHexbin() {
  const olympians = await d3.csv<any>("data/athletes.csv", d3.autoType);
  return pointermove(Plot.hexagon(olympians, Plot.hexbin({r: "count"}, {x: "weight", y: "height", tip: true})).plot());
}

export async function tipLine() {
  const aapl = await d3.csv<any>("data/aapl.csv", d3.autoType);
  return pointermove(Plot.lineY(aapl, {x: "Date", y: "Close", tip: true}).plot());
}
...
export async function tipRaster() {
  ...
  return pointermove(
    Plot.plot({
      ...
      marks: [
        Plot.raster(ca55, {x: "GRID_EAST", y: "GRID_NORTH", fill: "MAG_IGRF90", interpolate: "nearest", tip: true})
      ]
    })
  );
}
...
export async function tipRule() {
  const penguins = await d3.csv<any>("data/penguins.csv", d3.autoType);
  return pointermove(Plot.ruleX(penguins, {x: "body_mass_g", tip: true}).plot());
}
