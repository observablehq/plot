...
export default async function() {
  ...
  const VX = Float64Array.from(data, () => (random() - 0.5 + Math.sign(random() - 0.5)) * 0.001);
  const VY = Float64Array.from(data, () => (random() - 0.5 + Math.sign(random() - 0.5)) * 0.001);
