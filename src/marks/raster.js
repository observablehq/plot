...
function mixNumber3(a, ca, b, cb, c, cc) {
  return ca * a + cb * b + cc * c;
}

function mixerRandom2(random = Math.random) {
  return (a, b) => (random() < 0.5 ? a : b);
}

function mixerRandom3(random = Math.random) {
  return (a, ca, b, cb, c) => {
    const u = random();
...
function mixer2(F) {
  return isNumeric(F) || isTemporal(F) ? mixNumber2 : mixerRandom2(randomLcg());
}

function mixer3(F) {
  return isNumeric(F) || isTemporal(F) ? mixNumber3 : mixerRandom3(randomLcg());
}
