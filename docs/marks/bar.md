...
For required channels, see [barX](#barX) and [barY](#barY). The bar mark supports the [standard mark options](../features/marks.md). As a rect-like mark, the bar mark supports [insets](../features/marks.md#insets) and [rounded corners](./rect.md#rounded-corners). The **stroke** defaults to *none*. The **fill** defaults to *currentColor* if the stroke is *none*, and to *none* otherwise.
