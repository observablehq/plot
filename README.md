...
The *scale*.interval option will create an ordinal quantized domain—an array of equally spaced values spanning the extent of the defined values associated with the scale. An interval—such as d3.utcMonth—must expose an *interval*.floor(*value*), *interval*.offset(*value*), and *interval*.range(*start*, *stop*) functions. The option can also be specified as a number, in which case it will be promoted to an interval with the given step.
